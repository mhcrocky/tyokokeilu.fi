<div class="bravo-list-item bravo-list-job @if(!$rows->count()) not-found @endif">
    @if($rows->count())
        <div class="text-paginate">
            <h2 class="text">
                @if($rows->total() > 1)
                    {{ __(":count Jobs found",['count'=>$rows->total()]) }}
                @else
    ...
    @else
        <div class="not-found-box">
            <h3 class="n-title">{{__("We couldn't find any Jobs.")}}</h3>
