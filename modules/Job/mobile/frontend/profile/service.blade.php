...
@if(!empty($services) and $services->total())
    <div class="bravo-profile-list-services">
        ...
        <div class="container">
            ...
            @else
                <div class="text-center mt30"><a class="btn btn-sm btn-primary" href="{{route('user.profile.services',['id'=>$user->id,'type'=>'Job'])}}">{{__('View all (:total)',['total'=>$services->total()])}}</a></div>
            @endif
        </div>
    </div>
@endif
