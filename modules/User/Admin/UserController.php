    ...
    public function store(Request $request, $id)
    {
        ...
        if ($row->save()) {
            ...
            if (isset($permissions) && count($permissions) > 0) {
                if (in_array('job', $permissions)) {
                    $row->givePermissionTo('job_view');
                    $row->givePermissionTo('job_create');
                    $row->givePermissionTo('job_update');
                    $row->givePermissionTo('job_delete');
                }
                else {
                    $row->revokePermissionTo('job_view');
                    $row->revokePermissionTo('job_create');
                    $row->revokePermissionTo('job_update');
                    $row->revokePermissionTo('job_delete');
                }
            }
